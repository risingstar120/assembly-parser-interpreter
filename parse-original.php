...
class Validators
{
    ...
    {
        return isset($line) && strtolower($line) === ".ippcode24";
    }
...
class InputReader
{
     ...
     * @return array
     */
    public function get_input($file_name)
    {
        $file_handle = fopen($file_name, 'r');
        while (($line = $this->format_line(fgets($file_handle))) !== false) {
...
class XMLGenerator
{
    ...
    public function __construct()
    {
        ...
        $this->program->setAttribute("language", "IPPcode24");
    }
...
if (isset($options["help"])) {
    ...
    echo "
   \rWelcome to IPPCode24 parser!\n
   \rScript takes IPPCode24 as input, creates XML representation (encoding UTF-8)\nand sends it to output.
...
$input = $reader->get_input($options["input"]);
